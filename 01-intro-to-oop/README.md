# 01. Увод в ООП. Енумерации, пространства от имена. Структури.
...
```
<img src="assets/union.jpg" alt="union" width="600" />
...
Характеристики: 
- Всички елементи са директно съхранени в паметта.
- Задължително се извиква конструктор по подразбиране за всеки елемент.

Памет: 
- Ако има 10 елемента: $10 * sizeof(A)$  без допълнителни указатели.

Недостатъци: 
- Бавно преместване (*shifting*) на елементите:

Предимства: 
- Отлично **cache locality** – обектите са разположени плътно в паметта, което ускорява достъпа.
...
Характеристики:
- Масивът съхранява указатели, не самите обекти.
- Не е нужен конструктор по подразбиране, освен ако не създаваме самите обекти вътре.

Памет: 
- Ако има 10 елемента: 10 * sizeof(A*) за указателите , както и още 10 * sizeof(A) за обектите (на друго място в паметта).

Предимства: 
- Бързо преместване (shifting) – местят се само указатели.

Недостатъци: 
- По-слабо cache locality, тъй като обектите са разпиляни в паметта.
- Трябва ръчно управление на паметта – new, delete, възможност за изтичане на памет (memory leaks).
